type t = {
  ...
  avatar: unit => string,
  email: (option(string), option(string), option(string)) => string,
  ...
  mac: unit => string,
  password:
    (option(int), option(bool), option(Js.Re.t), option(string)) => string,
};
...
let email = (~firstName=?, ~lastName=?, ~provider=?, ()) =>
  fakers.email(firstName, lastName, provider);
...
let password = (~length=?, ~memorable=?, ~pattern=?, ~prefix=?, ()) =>
  fakers.password(length, memorable, pattern, prefix);
